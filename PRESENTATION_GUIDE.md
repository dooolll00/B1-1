...
이 문서는 2026-09-16에 검토한 미션 전문과 현재 코드·배포 결과를 기준으로 정리했습니다. **2장은 5~7분 요약 발표**, **7~10장은 자세한 설명과 요구사항 전체 확인용**입니다. 짧은 대본만 읽으면 모든 세부 항목을 다룰 수 없으므로 질의응답 준비에는 전체 확인표도 사용하세요.
...
- 저장소는 [B1-1](https://github.com/dooolll00/B1-1), 제출 URL은 [Pages](https://dooolll00.github.io/B1-1/)입니다. 2026-09-16 최신 코드의 배포와 공개 사이트 동작을 확인했습니다. 모의 API 시연은 로컬 사이트에서 진행합니다.
...
화면에서 여섯 구역과 각 구역의 역할을 짧게 보여 줍니다.
...
> loadProjects는 상태를 loading으로 바꾸고 로딩 화면을 그립니다. 유효한 캐시가 있으면 재사용하고, 없거나 재시도한 경우 fetchAllRepos에서 fetch와 async/await로 GitHub 응답을 기다린 뒤 checkRepoResponse에서 response.ok를 확인합니다. 정상 데이터는 repos에 저장하고 success로 바꿉니다. renderProjects는 map(createProjectCard)로 저장소 객체를 카드 문자열로 변환하고 join으로 합쳐 화면에 넣습니다.
>
...
> 입력 이벤트가 발생하면 값을 state.form.values에 저장합니다. validateField는 trim으로 공백만 있는 값을 거르고 이메일 형식도 확인합니다. 오류 문구는 errors에 저장하고 renderForm이 각 필드 근처에 표시합니다. touched로 검사 시점을 기억해, 필드를 떠났거나 제출한 뒤부터 입력 중에도 오류를 검사합니다.
>
> 제출할 때 preventDefault로 기본 이동을 막고 모든 필드를 검사합니다. 오류가 있으면 오류가 있는 첫 입력칸으로 포커스를 옮기고, 없으면 검증 성공을 표시합니다. 이 미션의 실제 이메일 전송은 선택 과제라서 현재는 전송하지 않습니다.
...
>
> 정상 화면뿐 아니라 빈 배열, 403, 네트워크 실패, 재시도, 손상된 캐시도 Chrome 자동 검증으로 확인했습니다. 미션의 핵심은 상태를 바꾸는 것과 화면을 갱신하는 것을 연결하는 것이었습니다. 이 프로젝트에서는 상태 변경 뒤 렌더링 함수를 직접 호출해 화면을 갱신합니다.

맨 위 버튼과 README의 자동 검사·스크린샷 항목을 보여 줍니다. 현재 상태는 다음처럼 설명합니다.

> 사이트는 GitHub Pages로 제공합니다. 디자인과 기능을 유지하면서 중복 코드를 정리했고, 2026년 9월 16일 Chrome에서 모의 API 검사 27개와 실제 API·공개 사이트 검사 2개가 통과했습니다. 이후 최신 코드를 배포하고 공개 파일이 로컬과 일치하는지 확인했습니다. 배포된 페이지에서도 테마 저장·폼·메뉴·스크롤·반응형·실제 API 동작을 다시 검사했습니다.

검증 환경은 Chrome 146.0.7680.165입니다. 자세한 검사 범위는 9장에서 확인합니다.
...
실제 버튼을 누른 뒤 “어떤 값이 바뀌었고 어떤 함수가 어떤 요소를 바꿨는가”를 말해 봅니다. 준비 여부는 10장의 확인표 한 곳에 표시합니다.
...
아래는 현재 main.js에서 사용하는 배열 처리 코드입니다. renderProjects 함수의 일부를 발췌했으므로 Console에 따로 실행하는 코드는 아닙니다.

```javascript
// renderProjects: 선택한 언어만 남기고 카드 HTML을 만듭니다.
...
**추가 기능:** readProjectCache는 5분 캐시를 읽고, fetchAllRepos는 100개 단위로 다음 페이지를 조회합니다. AbortController는 전체 조회가 15초를 넘으면 중단합니다. 캐시는 반복 요청을 줄이고, 페이지 조회는 100개를 넘는 저장소도 표시하며, 시간 제한은 응답을 무한히 기다리지 않게 합니다.
...
`errors`는 오류 문구, `touched`는 필드를 떠났거나 제출을 시도하여 검사 대상이 된 입력칸, `success`는 검증 성공 여부입니다. 제출할 때는 모든 입력칸을 검사 대상으로 표시합니다. 처음부터 오류를 표시하지 않으며, 입력을 수정하면 이전 성공 안내가 사라집니다. `novalidate`로 브라우저 기본 검증 팝업 대신 직접 만든 오류 UI를 사용하고, JS에서 필수값을 검사합니다.

`renderForm`은 처음에 선택해 둔 `fieldErrors`, `messageCount`, `formStatus`를 재사용합니다. 화면을 갱신할 때마다 같은 요소를 다시 찾지 않습니다.

**시연:** 빈 제출 → 공백만 입력 → 잘못된 이메일 → 정상 입력 → 성공 후 수정. 오류가 있는 첫 입력칸으로 포커스가 이동하는 것도 보여 줍니다. 이름이 정상이고 이메일이 잘못됐다면 이메일 입력칸으로 이동합니다.
...
| --- | --- | --- | --- |
| 테마 | 테마 버튼 클릭 | state.theme, state.explicitTheme | renderTheme → html data-theme, 버튼 안내, CSS 색상 |
| 메뉴 | 햄버거 클릭 등 | state.menuOpen | renderMenu → active, aria-expanded |
| API | 최초 로드 또는 재시도 | state.projects의 status/repos/error/filter | renderProjects → 로딩·카드·오류·빈 안내 |
| 폼 | input/blur/submit | state.form의 values/errors/touched/success | renderForm → 오류·글자 수·성공 안내 |
| 필터(선택) | 언어 버튼 클릭 | state.projects.filter | 버튼 선택 상태 갱신, renderProjects → 해당 언어 카드 |

`setMenuOpen`은 메뉴 상태를 바꾸고 `renderMenu`를 호출합니다. 시스템 테마는 저장된 선택이 없을 때 초기 테마에 반영하며, 사용자가 직접 테마를 선택하기 전까지 시스템 설정 변경도 따라갑니다. 직접 선택하면 `state.explicitTheme`이 true가 되어 시스템 변경보다 사용자 선택을 우선합니다.
...
2026-09-16 현재 코드에 대해 확인한 결과입니다. 과거 작업 과정은 WORK_LOG.md에 보관합니다.

| 검사 대상 | 확인된 결과 | 검사 조건 |
| --- | --- | --- |
| 로컬 기능·실제 API·당시 공개본 | 자동 검사 29개 통과 | Chrome 146.0.7680.165, 모의 API 검사 27개 + 실제 API 검사 2개 |
| 코드 정리 전후 비교 | 30개 상태 비교 일치, 전체 화면 캡처 18개 일치 | 320/375/767/768/1024/1440px의 두 테마 및 모바일 폼 상태, 동일 모의 데이터·폰트 차단·렌더링 조건 |
| 최신 코드 배포 후 공개본 | Pages 성공, 공개 HTML·CSS·JS와 로컬 파일 일치, 주요 기능 정상 | 테마 저장·폼·메뉴·Footer 포커스·맨 위 이동·반응형·이미지·실제 저장소 8개 확인 |

저장소 개수는 검사 당시 값이며 실제 GitHub 데이터에 따라 달라집니다. 검사한 Chrome 버전이 현재 최신 버전인지는 별도 확인이 필요합니다. `tests/review.py`는 사이트에 로드되지 않는 개발용 검사 도구입니다.

제출 자료는 GitHub 저장소 URL, GitHub Pages URL, 데스크톱·모바일·다크 모드 스크린샷입니다. 현재 제출용 캡처는 2026-09-07 같은 디자인으로 촬영한 이미지이며, 현재와 저장소 목록이 다를 수 있습니다.
...
| [ ] | 공개본 반응형·인터랙션·API·폼 정상 동작 | 9장 | 2026-09-16 최신 배포 후 확인 완료 |
